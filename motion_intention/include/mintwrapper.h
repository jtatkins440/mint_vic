...
class MIntWrapper{
	...
	void forwardFitSpline(Eigen::ArrayXf current_state, std::deque<Eigen::ArrayXf> input);
	void forwardFitSpline(Eigen::ArrayXf current_state, Eigen::ArrayXXf input);
	...
	private:
    	...
    	Eigen::ArrayXXf _forward(Eigen::ArrayXXf input);
		MIntSpline mintspline;
};


Eigen::ArrayXXf MIntWrapper::_forward(Eigen::ArrayXXf input)
{
...
{
  output_a = _forward(input);
	return output_a;
};

void MIntWrapper::forwardFitSpline(Eigen::ArrayXf current_state, std::deque<Eigen::ArrayXf> input)
{
	auto pred_pos = forward(input);
	mintspline.updateSpline(current_state, pred_pos);
};

void MIntWrapper::forwardFitSpline(Eigen::ArrayXf current_state, Eigen::ArrayXXf input)
{
	auto pred_pos = forward(input);
	mintspline.updateSpline(current_state, pred_pos);
};
